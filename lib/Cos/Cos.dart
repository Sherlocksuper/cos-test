...
class CosController extends GetxController {
  ...
  createTask(srcPath) async {
    //如果late的transfer还没有被赋值，就先初始化

    CosTransferManger transferManager = Cos().getDefaultTransferManger();

    //根据srcPath判断后缀
    String suffix = srcPath.substring(srcPath.lastIndexOf(".") + 1);
    TransferTask transferTask = await transferManager.upload(bucket, "$cosPath/${DateTime.now()}.$suffix",
        filePath: srcPath,
        uploadId: _uploadId,
  ...
  showMyFile() async {
    try {
      ...
      for (var contents in bucketContents.contentsList) {
        print(contents?.key);
      }

      return bucketContents.contentsList;
    } catch (e) {
      print(e);
    }
  }
